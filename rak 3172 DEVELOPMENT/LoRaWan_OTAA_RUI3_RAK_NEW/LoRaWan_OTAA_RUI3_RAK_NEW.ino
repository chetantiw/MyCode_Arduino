...
void uplink_routine()
{
 ...
 json["T"] = String(TempC);
 json["H"] = String(HumidityC);
 
 char charArray[128];
 int len = serializeJson(json, charArray);
 char buildBuffer[2] = {0};
 uint8_t compositionBuffer[len*2+1] = {0};  // this will hold a string we build
    
 for (int i = 0; i < len; i++) {
     sprintf( buildBuffer, "%02X ", (uint8_t)charArray[i]);
     compositionBuffer[i*2] = (uint8_t)buildBuffer[0];
     compositionBuffer[i*2+1] = (uint8_t)buildBuffer[1];
 }
 /** Send the data package */
  
  //Serial.println(compositionBuffer);
  if (api.lorawan.send(sizeof(compositionBuffer), compositionBuffer, 2, true, 1)) {
